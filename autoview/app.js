...
function render() {
  app.innerHTML = `
    <div class="app-shell">
      ${renderSidebar()}
      ${!state.sidebarCollapsed ? '<div class="sidebar-backdrop" data-action="toggle-sidebar"></div>' : ''}
      <div class="main">
        ${renderTopbar()}
        ${renderMainBody()}
      </div>
...
function bindEvents() {
  ...
  document.querySelectorAll('[data-plan-menu]').forEach(el => {
    el.addEventListener('click', (e) => {
      ...
      const name = projects[pi] && projects[pi].plans[idx];
      const slug = (name || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
      const r = el.getBoundingClientRect();
      showCtxMenu(r.left, r.bottom + 4, [
        { id: 'rename', icon: ic('edit'), label: 'Rename', action: () => { state.renamingPlan = { pi, idx }; render(); } },
        { id: 'share', icon: ic('share'), label: 'Share', action: () => showTeamShare(name, `Invite teammates to collaborate on “${escapeHtml(name)}”.`, `www.autoview.app/c/${slug}`) },
        { id: 'dup', icon: ic('copy'), label: 'Duplicate', action: () => toast('Duplicated') },
        { divider: true },
